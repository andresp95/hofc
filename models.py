...
class Material(db.Model):
    ...
    express_product_links = db.relationship("ExpressProductMaterial", back_populates="material")
...
class Order(db.Model):
    ...
    express_products = db.relationship("ExpressProduct", back_populates="order", cascade="all, delete-orphan")
...
class OrderItem(db.Model):
    ...
    express_product_id = db.Column(
        db.Integer,
        db.ForeignKey("express_products.id", ondelete="CASCADE"),
    )
    ...
    express_product = db.relationship("ExpressProduct", back_populates="order_items")

    __table_args__ = (
        CheckConstraint("unit_price >= 0", name="ck_order_items_unit_price_nonnegative"),
        CheckConstraint("quantity > 0", name="ck_order_items_quantity_positive"),
        CheckConstraint(
            "(product_id IS NOT NULL) != (express_product_id IS NOT NULL)",
            name="ck_order_items_one_source",
        ),
    )


class ExpressProduct(db.Model):
    __tablename__ = "express_products"
    ...
    order = db.relationship("Order", back_populates="express_products")
    material_links = db.relationship(
        "ExpressProductMaterial",
        back_populates="express_product",
        cascade="all, delete-orphan",
    )
    order_items = db.relationship("OrderItem", back_populates="express_product")


class ExpressProductMaterial(db.Model):
    __tablename__ = "express_product_materials"
    ...
    express_product = db.relationship("ExpressProduct", back_populates="material_links")
    material = db.relationship("Material", back_populates="express_product_links")

    __table_args__ = (
        CheckConstraint("quantity > 0", name="ck_express_product_materials_quantity_positive"),
        UniqueConstraint(
            "express_product_id",
            "material_id",
            name="uq_express_product_materials_product_material",
        ),
    )
